...
public class Plugin : BaseUnityPlugin
{
    ...
    private void InitConfiguration()
    {
        ...
        SpendHalfEnergy = Config.Bind("5. Gameplay", "Spend Half Energy", true, new ConfigDescription("Reduce energy consumption by half", null, new ConfigurationManagerAttributes {Order = 43}));
        SpendHalfGratitude = Config.Bind("5. Gameplay", "Spend Half Gratitude", true, new ConfigDescription("Reduce gratitude consumption by half", null, new ConfigurationManagerAttributes {Order = 42}));
        SpendHalfSanity = Config.Bind("5. Gameplay", "Spend Half Sanity", true, new ConfigDescription("Reduce sanity consumption by half", null, new ConfigurationManagerAttributes {Order = 41}));

        UnlimitedEnergy = Config.Bind("6. Unlimited Stats", "Unlimited Energy", false, new ConfigDescription("Unlimited energy. This overrides Spend Half Energy.", null, new ConfigurationManagerAttributes { Order = 40 }));
        UnlimitedEnergy.SettingChanged += (_, _) =>
        {
            if(UnlimitedEnergy.Value)
                SpendHalfEnergy.Value = false;
        };
        UnlimitedGratitude = Config.Bind("6. Unlimited Stats", "Unlimited Gratitude", false, new ConfigDescription("Unlimited gratitude. This overrides Spend Half Gratitude.", null, new ConfigurationManagerAttributes { Order = 39 }));
        UnlimitedGratitude.SettingChanged += (_, _) =>
        {
            if(UnlimitedGratitude.Value)
                SpendHalfGratitude.Value = false;
        }; 
        UnlimitedSanity = Config.Bind("6. Unlimited Stats", "Unlimited Sanity", false, new ConfigDescription("Unlimited sanity. This overrides Spend Half Sanity.", null, new ConfigurationManagerAttributes { Order = 38 }));
